...
async function odooCallKw(model: string, method: string, args: any[], kwargs: any): Promise<any[]> {
  try {
...
async function odooGetProduits(): Promise<any[]> {
  ...
  if (!auth) return [];
  return odooCallKw("product.template", "search_read",
    [[["sale_ok", "=", true], ["active", "=", true]]],
    { fields: ["name", "list_price", "description_sale", "categ_id", "qty_available"], limit: 100 }
  );
}

async function odooLoginClient(email: string, password: string): Promise<any> {
  try {
    const res = await fetch(`${ODOO_URL}/web/session/authenticate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const data = await res.json();
    return data.result?.uid ? data.result : null;
  } catch { return null; }
}

async function odooCreateVente(clientOdooId: number, produits: any[]): Promise<boolean> {
  const auth = await odooAuthenticate();
  if (!auth) return false;
  try {
    await odooCallKw("sale.order", "create",
      [{ partner_id: clientOdooId, order_line: produits.filter(p => p.odoo_id).map(p => [0, 0, { product_id: p.odoo_id, product_uom_qty: 1, price_unit: p.prix }]) }],
      {}
    );
    return true;
  } catch { return false; }
...
function wsConnect() {
  try {
    ...
    ws.onopen = () => {
      console.log("✅ WebSocket DSM connecté");
      clearTimeout(wsReconnectTimer);
      wsNotify("connexion", { status: "connecte" });
    };
    ws.onmessage = (event: any) => {
      try {
        const data = JSON.parse(event.data);
        wsNotify(data.type, data);
        wsNotify("*", data);
      } catch {}
    };
    ws.onclose = () => {
      wsNotify("deconnexion", { status: "deconnecte" });
...
function wsOn(type: string, cb: Function) {
  ...
  return () => {
    const list = wsListeners.get(type) || [];
    wsListeners.set(type, list.filter(x => x !== cb));
  };
}
...
const LIVRES_STATIQUES = [
  { titre: "Les Misérables", auteur: "Victor Hugo", prix: 18.9, emoji: "📗", genre: "roman", desc: "Une fresque humaniste inoubliable.", pages: 1900, annee: 1862, note: 4.9 },
  { titre: "L'Étranger", auteur: "Albert Camus", prix: 8.5, emoji: "📘", genre: "roman", desc: "Le roman fondateur de l'absurde.", pages: 186, annee: 1942, note: 4.7 },
  { titre: "Pars vite et reviens tard", auteur: "Fred Vargas", prix: 9.2, emoji: "🔍", genre: "policier", desc: "Commissaire Adamsberg face à une menace.", pages: 368, annee: 2001, note: 4.8 },
  { titre: "Dune", auteur: "Frank Herbert", prix: 14.9, emoji: "🌑", genre: "sci-fi", desc: "L'épopée épique d'Arrakis.", pages: 896, annee: 1965, note: 4.9 },
  { titre: "Harry Potter T.1", auteur: "J.K. Rowling", prix: 13.9, emoji: "🧙", genre: "jeunesse", desc: "Le début d'une aventure magique.", pages: 320, annee: 1997, note: 5.0 },
  { titre: "Le Petit Prince", auteur: "Saint-Exupéry", prix: 7.5, emoji: "🌹", genre: "jeunesse", desc: "Le conte philosophique le plus lu.", pages: 96, annee: 1943, note: 4.9 },
  { titre: "Fondation", auteur: "Isaac Asimov", prix: 11.5, emoji: "🚀", genre: "sci-fi", desc: "Le cycle le plus ambitieux de la SF.", pages: 400, annee: 1951, note: 4.9 },
  { titre: "Tintin au Tibet", auteur: "Hergé", prix: 14.5, emoji: "🎨", genre: "bd", desc: "L'aventure la plus émouvante de Tintin.", pages: 62, annee: 1960, note: 4.9 },
];
...
function SourceBadge({ source }: { source: "odoo" | "supabase" | "local" }) {
  ...
  return (
    <View style={{ backgroundColor: c.color + "20", borderRadius: 99, paddingHorizontal: 10, paddingVertical: 3, alignSelf: "flex-start", marginTop: 4 }}>
...
/* ══════════════════════════════════════
   BADGE WEBSOCKET
══════════════════════════════════════ */
function WsBadge({ connecte }: { connecte: boolean }) {
  return (
    <View style={{ backgroundColor: connecte ? "#27AE6020" : "#E74C3C20", borderRadius: 99, paddingHorizontal: 10, paddingVertical: 3, alignSelf: "flex-start" }}>
      <Text style={{ fontSize: 10, color: connecte ? "#27AE60" : "#E74C3C", fontWeight: "bold" }}>
        {connecte ? "⚡ Temps réel actif" : "📴 Hors ligne"}
      </Text>
    </View>
  );
}
    ...
    <View style={{ alignItems: "center", paddingVertical: 20, paddingHorizontal: 16 }}>
      <View style={qrStyles.qrContainer}>
        <View style={qrStyles.qrBox}>
          <QRCode value={numCarte} size={120} color="#05102A" backgroundColor="#fff" />
        </View>
        <View style={qrStyles.qrInfo}>
          <Text style={qrStyles.qrNom}>{prenom} {nom}</Text>
          <Text style={qrStyles.qrNum}>N° {numCarte}</Text>
          <Text style={qrStyles.qrHint}>Scannez en caisse pour valider vos points</Text>
        </View>
      </View>
      <View style={qrStyles.barcodeRow}>
        {bars.map((w, i) => (
          <View key={i} style={{ width: w * 2, height: 50, backgroundColor: i % 2 === 0 ? "#111" : "#fff" }} />
        ))}
      </View>
...
const qrStyles = StyleSheet.create({
  qrContainer: { flexDirection: "row", alignItems: "center", gap: 16, backgroundColor: "#fff", borderRadius: 16, padding: 16, marginBottom: 16, width: "100%", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  qrBox: { padding: 8, backgroundColor: "#fff", borderRadius: 12, borderWidth: 1, borderColor: "#E0EDFF" },
  qrInfo: { flex: 1 },
  qrNom: { fontSize: 14, fontWeight: "bold", color: "#05102A", marginBottom: 4 },
  qrNum: { fontSize: 13, color: "#1A6FFF", fontWeight: "bold", letterSpacing: 2, marginBottom: 6 },
  qrHint: { fontSize: 10, color: "#8AAABF", lineHeight: 14 },
  barcodeRow: { flexDirection: "row", height: 50, overflow: "hidden" },
  barcodeNum: { fontSize: 12, letterSpacing: 3, marginTop: 8, color: "#333", fontWeight: "bold" },
});
...
function EcranConnexion({ onLogin }: { onLogin: (c: any) => void }) {
  ...
  const connexion = async () => {
    ...
    try {
      const { data, error } = await supabase.from("clients").select("*").eq("email", email).eq("mdp", mdp).single();
      if (error || !data) {
  ...
  const inscription = async () => {
    ...
    setLoading(true); setErreur("");
    try {
      const { data: exist } = await supabase.from("clients").select("id").eq("email", email).single();
      if (exist) { setErreur("❌ Cet email est déjà utilisé"); setLoading(false); return; }
      const numCarte = String(Math.floor(1000 + Math.random() * 9000));
      const depuis = new Date().getFullYear().toString();
      const { data, error } = await supabase.from("clients").insert({
        prenom, nom, email, mdp, niveau: "Bronze", points: 50,
        num_carte: numCarte, genre_favori: genre, auteur_favori: "",
        depuis, date_naissance: anniversaire || null,
      }).select().single();
      if (error || !data) {
        setErreur("❌ Erreur lors de l'inscription");
      } else {
        await supabase.from("notifications").insert({
          client_id: data.id, titre: "🎉 Bienvenue chez DSM !",
          message: `Bonjour ${prenom} ! Votre carte est prête. +50 pts de bienvenue !`, lu: false,
        });
        await envoyerEmailBienvenue(email, prenom, numCarte);
        Alert.alert("🎉 Bienvenue !", `Votre carte DSM est activée !\nN° ${numCarte}\n+50 pts offerts !`);
        onLogin(data);
      }
    } catch { setErreur("❌ Erreur de connexion"); }
    setLoading(false);
  ...
  return (
    <ScrollView contentContainerStyle={s.loginContainer}>
      ...
      <View style={s.toggleRow}>
        <TouchableOpacity style={[s.toggleBtn, mode === "login" && s.toggleBtnActif]} onPress={() => { setMode("login"); setErreur(""); }}>
          <Text style={[s.toggleTxt, mode === "login" && s.toggleTxtActif]}>Connexion</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.toggleBtn, mode === "inscription" && s.toggleBtnActif]} onPress={() => { setMode("inscription"); setErreur(""); }}>
          <Text style={[s.toggleTxt, mode === "inscription" && s.toggleTxtActif]}>Inscription</Text>
        </TouchableOpacity>
      </View>
      <View style={s.form}>
        {mode === "login" && <>
          <TextInput style={s.input} placeholder="Email" placeholderTextColor="#7AAAD0" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />
          <TextInput style={s.input} placeholder="Mot de passe" placeholderTextColor="#7AAAD0" value={mdp} onChangeText={setMdp} secureTextEntry />
        ...
        {mode === "inscription" && <>
          ...
          <TextInput style={s.input} placeholder="Email" placeholderTextColor="#7AAAD0" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />
          <TextInput style={s.input} placeholder="Mot de passe" placeholderTextColor="#7AAAD0" value={mdp} onChangeText={setMdp} secureTextEntry />
          <TextInput style={s.input} placeholder="Date de naissance (JJ/MM/AAAA)" placeholderTextColor="#7AAAD0" value={anniversaire} onChangeText={setAnniversaire} />
      ...
      <ScrollView>
        <View style={s.detailHero}>
          ...
          <View style={s.detailBadgeRow}>
            <View style={s.detailBadge}><Text style={s.detailBadgeTxt}>{livre.genre}</Text></View>
            {livre.pages > 0 && <View style={s.detailBadge}><Text style={s.detailBadgeTxt}>{livre.pages} pages</Text></View>}
            {livre.annee > 0 && <View style={s.detailBadge}><Text style={s.detailBadgeTxt}>{livre.annee}</Text></View>}
          </View>
          <View style={{ flexDirection: "row", gap: 4, marginTop: 8 }}>
            {"★★★★★".split("").map((_, i) => (
              <Text key={i} style={{ color: i < Math.floor(livre.note || 4) ? "#F5A623" : "#ccc", fontSize: 20 }}>★</Text>
            ))}
            <Text style={{ color: "#8AAABF", fontSize: 14, marginLeft: 4 }}>{livre.note || 4.5}/5</Text>
          </View>
        </View>
        <View style={{ padding: 20 }}>
          <Text style={s.detailDescTitre}>Description</Text>
          <Text style={s.detailDesc}>{livre.desc || "Aucune description disponible."}</Text>
          <View style={s.detailPrixBox}>
            ...
            <View style={{ alignItems: "flex-end" }}>
              ...
              <Text style={{ fontSize: 20, color: "#F5A623", fontWeight: "bold" }}>+{Math.round(livre.prix * 10)} pts</Text>
            </View>
          </View>
          <TouchableOpacity style={s.detailAcheterBtn} onPress={onAchat}>
            <Text style={s.detailAcheterTxt}>🛍️ Ajouter au panier</Text>
          </TouchableOpacity>
          <TouchableOpacity style={s.detailWhatsappBtn} onPress={() => Linking.openURL(`whatsapp://send?text=Je recommande "${livre.titre}" de ${livre.auteur} — ${livre.prix.toFixed(2)} DH chez DSM Librairie 📚`)}>
...
/* ══════════════════════════════════════
   ONGLET ACCUEIL
══════════════════════════════════════ */
function OngletAccueil({ client, onAnniversaire, wsConnecte }: { client: any; onAnniversaire: () => void; wsConnecte: boolean }) {
  ...
  useEffect(() => {
    const unsub = NetInfo.addEventListener((state) => { setEstEnLigne(state.isConnected ?? true); });
    return () => unsub();
  }, []);

  useEffect(() => {
    supabase.from("achats").select("*").eq("client_id", client.id).order("date_achat", { ascending: false }).limit(20).then(({ data }) => { if (data) setAchats(data); });
    if (client.date_naissance && isAnniversaire(client.date_naissance)) onAnniversaire();
  }, [client.id]);
  ...
  return (
    <ScrollView style={s.ongletContainer} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ margin: 12, marginBottom: 0 }}>
        <WsBadge connecte={wsConnecte} />
      </View>

      {!estEnLigne && (
        <View style={{ backgroundColor: "#FF6B35", padding: 10, flexDirection: "row", alignItems: "center", justifyContent: "center", borderRadius: 10, margin: 12 }}>
          <Text style={{ fontSize: 16, marginRight: 8 }}>📵</Text>
          <Text style={{ color: "#fff", fontWeight: "bold", fontSize: 13 }}>Mode hors ligne — Carte disponible en lecture seule</Text>
        </View>
      )}
      ...
      {client.date_naissance && isAnniversaire(client.date_naissance) && (
        <View style={s.anniversaireBanner}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.anniversaireTitre}>Joyeux anniversaire {client.prenom} !</Text>
            <Text style={s.anniversaireSous}>+100 pts offerts aujourd'hui 🎁</Text>
          </View>
        </View>
      )}
      ...
      <View style={{ marginHorizontal: 16, marginBottom: 16, backgroundColor: "#05102A", borderRadius: 16, padding: 14 }}>
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 8 }}>
          ...
          {actuel.nom === "Platine"
            ? <Text style={{ fontSize: 11, color: "#E5E4E2", fontWeight: "bold" }}>💎 Niveau maximum atteint !</Text>
            : <Text style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>encore <Text style={{ color: actuel.couleur, fontWeight: "bold" }}>{manquants} pts</Text> pour {suivant?.emoji} {suivant?.nom}</Text>
          }
        </View>
      </View>
      ...
      <View style={s.barcodeBloc}>
        <BarcodeVisuel numCarte={client.num_carte} prenom={client.prenom} nom={client.nom} />
      </View>
      ...
      {[
        ...
        <TouchableOpacity key={i} style={s.menuItem} onPress={() => { if (item.label === "Dernier ticket d'achat") setShowTicket(true); }}>
        ...
        <View style={s.ticketModal}>
          <View style={s.ticketContainer}>
            ...
            <View style={s.ticketTotal}>
              ...
              <Text style={s.ticketTotalVal}>{achats[0]?.prix?.toFixed(2)} DH</Text>
            </View>
            <View style={s.ticketPtsRow}>
              <Text style={s.ticketPtsLbl}>Points gagnés</Text>
              <Text style={s.ticketPtsVal}>+{achats[0]?.points_gagnes} pts</Text>
            </View>
            <View style={s.ticketSep} />
            <View style={{ alignItems: "center", marginVertical: 12 }}>
              <QRCode value={client.num_carte} size={80} color="#05102A" backgroundColor="#fff" />
              <Text style={{ fontSize: 10, color: "#666", letterSpacing: 2, marginTop: 6 }}>•••• •••• •••• {client.num_carte}</Text>
            </View>
            <Text style={s.ticketMerci}>Merci de votre visite !</Text>
            <TouchableOpacity style={s.ticketFermer} onPress={() => setShowTicket(false)}>
      ...
      <Modal visible={showHistorique} animationType="slide" onRequestClose={() => setShowHistorique(false)}>
        <View style={{ flex: 1, backgroundColor: "#05102A" }}>
          <View style={{ backgroundColor: "#0A1F4E", padding: 20, paddingTop: 50, flexDirection: "row", alignItems: "center" }}>
            ...
            <View>
              <Text style={{ fontSize: 18, color: "#fff", fontWeight: "bold" }}>Historique des achats</Text>
              <Text style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>{achats.length} achats au total</Text>
            </View>
          </View>
          <View style={{ flexDirection: "row", margin: 16, gap: 10 }}>
            {[
              { val: achats.reduce((s, a) => s + (a.prix || 0), 0).toFixed(0) + " DH", lbl: "Total dépensé", color: "#FFD080" },
              { val: achats.reduce((s, a) => s + (a.points_gagnes || 0), 0) + " pts", lbl: "Points gagnés", color: "#4CAF50" },
              { val: String(achats.length), lbl: "Livres achetés", color: "#2E86FF" },
            ].map((item, i) => (
              <View key={i} style={{ flex: 1, backgroundColor: "#0A1F4E", borderRadius: 14, padding: 14, alignItems: "center" }}>
                <Text style={{ fontSize: 22, color: item.color, fontWeight: "bold" }}>{item.val}</Text>
                <Text style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginTop: 4 }}>{item.lbl}</Text>
              </View>
            ))}
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ paddingLeft: 16, marginBottom: 8, maxHeight: 44 }}>
            {["Tous", "Jan", "Fév", "Mar", "Avr", "Mai", "Jun", "Jul", "Aoû", "Sep", "Oct", "Nov", "Déc"].map((m, i) => (
              <TouchableOpacity key={m} onPress={() => setFiltreMois(i)} style={{ paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, marginRight: 8, backgroundColor: filtreMois === i ? "#2E86FF" : "rgba(255,255,255,0.08)" }}>
                <Text style={{ color: filtreMois === i ? "#fff" : "rgba(255,255,255,0.5)", fontWeight: "600", fontSize: 13 }}>{m}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          <ScrollView style={{ flex: 1, padding: 16 }}>
            {achats.filter((a) => filtreMois === 0 ? true : new Date(a.created_at).getMonth() + 1 === filtreMois).map((achat, i) => (
              <View key={i} style={{ backgroundColor: "#0A1F4E", borderRadius: 14, padding: 16, marginBottom: 12, flexDirection: "row", alignItems: "center" }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ color: "#fff", fontWeight: "bold", fontSize: 14 }} numberOfLines={1}>{achat.titre}</Text>
                  <Text style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, marginTop: 2 }}>{achat.auteur}</Text>
                  <Text style={{ color: "rgba(255,255,255,0.3)", fontSize: 11, marginTop: 4 }}>{new Date(achat.created_at).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</Text>
                </View>
                <View style={{ alignItems: "flex-end" }}>
                  <Text style={{ color: "#FFD080", fontWeight: "bold", fontSize: 15 }}>{achat.prix?.toFixed(2)} DH</Text>
                  <View style={{ backgroundColor: "rgba(76,175,80,0.15)", borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3, marginTop: 6 }}>
                    <Text style={{ color: "#4CAF50", fontSize: 11, fontWeight: "bold" }}>+{achat.points_gagnes} pts</Text>
                  </View>
                </View>
              </View>
            ))}
...
/* ══════════════════════════════════════
   ONGLET BOUTIQUE — Odoo 18 + Temps réel
══════════════════════════════════════ */
function OngletBoutique({ client, onAchat, onValider }: { client: any; onAchat: (pts: number, livres: any[], total: number) => void; onValider: (panier: any[]) => void }) {
  ...
  const [sourceData, setSourceData] = useState<"odoo" | "supabase" | "local">("local");
  ...
  useEffect(() => {
    const chargerLivres = async () => {
      ...
      try {
        ...
        if (produits.length > 0) {
          setLivres(produits.map((p: any) => ({
            ...
            pages: 0, annee: new Date().getFullYear(), note: 4.5,
            odoo_id: p.id, stock: p.qty_available || 0,
          })));
          setSourceData("odoo");
      ...
      // 2. Fallback Supabase
      try {
        const { data } = await supabase.from("produits").select("*").eq("disponible", true).order("titre");
        if (data && data.length > 0) {
          setLivres(data.map((p) => ({
            titre: p.titre || p.name, auteur: p.auteur || "Auteur inconnu",
            prix: parseFloat(p.prix) || 0, emoji: EMOJI_GENRE[p.genre?.toLowerCase()] || "📚",
            genre: p.genre || "roman", desc: p.description || "",
            pages: p.pages || 0, annee: p.annee || new Date().getFullYear(), note: p.note || 4.5,
          })));
    ...
    const unsubProduit = wsOn("produit_modifie", (data: any) => {
      const now = new Date().toLocaleTimeString("fr-FR");
      setDerniereMaj(`Mis à jour à ${now}`);

      setLivres((livresActuels) => {
        if (data.action === "create") {
          return [...livresActuels, {
            titre: data.name || data.nom,
            auteur: data.categ_id || "DSM",
            prix: data.list_price || data.prix || 0,
            emoji: "📚", genre: "roman",
            desc: data.description_sale || "",
            note: 4.5, odoo_id: data.id,
            stock: data.qty_available || 0,
          }];
        } else if (data.action === "update") {
          return livresActuels.map((l) =>
            l.odoo_id === data.id
              ? { ...l, titre: data.name || l.titre, prix: data.list_price || l.prix, stock: data.qty_available || l.stock }
              : l
          );
        } else if (data.action === "delete") {
          return livresActuels.filter((l) => l.odoo_id !== data.id);
        }
        return livresActuels;
      });
    });
  ...
  const livresFiltres = livres.filter((l) => {
    ...
    const matchRecherche = !recherche || l.titre.toLowerCase().includes(recherche.toLowerCase()) || l.auteur.toLowerCase().includes(recherche.toLowerCase());
    return matchGenre && matchRecherche;
  });

  if (livreDetail) {
    return <DetailLivre livre={livreDetail} onAchat={() => { setPanier((p) => [...p, livreDetail]); setLivreDetail(null); }} onBack={() => setLivreDetail(null)} />;
  }
  ...
  return (
    <View style={s.ongletContainer}>
        ...
        <View>
          ...
          {!loadingLivres && (
            <View style={{ flexDirection: "row", gap: 8, alignItems: "center", marginTop: 4 }}>
              <SourceBadge source={sourceData} />
              {derniereMaj ? <Text style={{ fontSize: 9, color: "#27AE60" }}>{derniereMaj}</Text> : null}
            </View>
          )}
        </View>
      ...
      <View style={s.rechercheBox}>
        <Text style={{ fontSize: 16, marginRight: 8 }}>🔍</Text>
        <TextInput style={s.rechercheInput} placeholder="Rechercher un livre, auteur..." placeholderTextColor="#8AAABF" value={recherche} onChangeText={setRecherche} />
        {recherche.length > 0 && <TouchableOpacity onPress={() => setRecherche("")}><Text style={{ color: "#8AAABF", fontSize: 18, marginLeft: 8 }}>✕</Text></TouchableOpacity>}
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 44, paddingLeft: 12 }}>
        {genres.map((g) => (
          <TouchableOpacity key={g} onPress={() => setGenreActif(g)} style={{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 99, backgroundColor: genreActif === g ? "#05102A" : "#EAF2FF", marginRight: 8 }}>
            <Text style={{ fontSize: 12, fontWeight: "700", color: genreActif === g ? "#FFD080" : "#4A6080" }}>
          ...
          {panier.length === 0 ? <Text style={s.panierVide}>Panier vide</Text> : <>
            {panier.map((livre, i) => (
              <View key={i} style={s.panierItem}>
                ...
                <Text style={{ color: "#05102A", fontWeight: "bold" }}>{livre.prix.toFixed(2)} DH</Text>
              </View>
            ))}
      ...
      <ScrollView>
        {loadingLivres ? (
          <View style={{ alignItems: "center", padding: 60 }}>
            ...
            <Text style={{ color: "#8AAABF", marginTop: 16, fontSize: 14 }}>Chargement du catalogue...</Text>
            <Text style={{ color: "#B0C4D8", marginTop: 6, fontSize: 12 }}>Synchronisation Odoo 18 ⚡</Text>
          </View>
        ) : (
          <View style={s.livresGrid}>
            {livresFiltres.map((livre, i) => (
              <TouchableOpacity key={i} style={s.livreCard} onPress={() => setLivreDetail(livre)}>
                <Text style={s.livreEmoji}>{livre.emoji}</Text>
                <Text style={s.livreTitre} numberOfLines={2}>{livre.titre}</Text>
                <Text style={s.livreAuteur} numberOfLines={1}>{livre.auteur}</Text>
                {livre.stock !== undefined && livre.stock <= 3 && livre.stock > 0 && (
                  <Text style={{ fontSize: 9, color: "#E74C3C", textAlign: "center", marginBottom: 2 }}>⚠️ Plus que {livre.stock} en stock</Text>
                )}
        ...
        {!loadingLivres && livresFiltres.length === 0 && (
          <View style={{ alignItems: "center", padding: 40 }}>
            ...
            <Text style={{ color: "#8AAABF", marginTop: 12, fontSize: 15 }}>Aucun livre trouvé</Text>
          </View>
        )}
      </ScrollView>
    </View>
  ...
  return (
    <ScrollView style={s.ongletContainer}>
      ...
      <View style={{ padding: 16 }}>
        {OFFRES.map((offre, i) => (
          <View key={i} style={[s.offreCard, { borderLeftColor: offre.couleur }]}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 8 }}>
              <View style={[s.offreIconBox, { backgroundColor: offre.couleur + "20" }]}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <Text style={{ fontSize: 11, color: "#8AAABF" }}>Expire le {offre.expire}</Text>
              <TouchableOpacity style={[s.offreBtn, { backgroundColor: offre.couleur }]}>
...
function OngletClassement({ client }: { client: any }) {
  ...
  useEffect(() => {
    supabase.from("clients").select("id,prenom,nom,points,niveau").order("points", { ascending: false }).limit(10).then(({ data }) => {
          ...
          classement.map((c, i) => {
            ...
            return (
              <View key={i} style={[s.classementItem, isMe && s.classementItemMe]}>
                <Text style={s.classementRang}>{medailles[i] || `${i + 1}`}</Text>
                <View style={s.classementAvatar}>
                  <Text style={{ color: "#fff", fontWeight: "bold", fontSize: 14 }}>{c.prenom[0]}{c.nom[0]}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.classementNom, isMe && { color: "#1A6FFF" }]}>{c.prenom} {c.nom} {isMe ? "(Moi)" : ""}</Text>
                  <Text style={s.classementNiveau}>{c.niveau}</Text>
                </View>
                <Text style={s.classementPts}>{c.points.toLocaleString()} pts</Text>
              </View>
            );
          })
        )}
...
function OngletNotifs({ client }: { client: any }) {
  ...
  useEffect(() => {
    supabase.from("notifications").select("*").eq("client_id", client.id).order("created_at", { ascending: false }).then(({ data }) => { if (data) setNotifs(data); });

    // Écouter les nouvelles notifications WebSocket
    const unsub = wsOn("notification", (data: any) => {
      if (!data.client_id || data.client_id === client.id) {
        setNotifs(n => [{
          id: Date.now(), titre: data.titre, message: data.message,
...
function OngletProfil({ client, onDeconnexion }: { client: any; onDeconnexion: () => void }) {
  const partagerCarte = async () => {
    try {
      await Linking.openURL(`whatsapp://send?text=Ma carte fidélité DSM Librairie 📚%0AN° : ${client.num_carte}%0ANiveau : ${client.niveau}%0APoints : ${client.points} pts`);
  ...
  return (
    <ScrollView style={s.ongletContainer}>
      <View style={s.profilHeader}>
        <View style={s.avatar}>
          <Text style={s.avatarTxt}>{client.prenom[0]}{client.nom[0]}</Text>
        </View>
        ...
        <TouchableOpacity style={s.whatsappBtn} onPress={partagerCarte}>
          <Text style={s.whatsappBtnTxt}>📱 Partager ma carte sur WhatsApp</Text>
        </TouchableOpacity>
      </View>
      <View style={{ padding: 16 }}>
        {[
          { icon: "📚", label: "Genre favori", val: client.genre_favori },
          { icon: "✍️", label: "Auteur favori", val: client.auteur_favori || "Non défini" },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubCo = wsOn("connexion", () => setWsConnecte(true));
    const unsubDeco = wsOn("deconnexion", () => setWsConnecte(false));
  ...
  // ── Écouter les points en temps réel ────────────
  useEffect(() => {
    if (!client) return;
    const unsub = wsOn("points_mis_a_jour", (data: any) => {
      if (data.dsm_num_carte === client.num_carte) {
        setClient((c: any) => ({ ...c, points: data.points_apres }));
        Alert.alert("⭐ Points mis à jour !", `+${data.points_gagnes || 0} pts\nNouveau solde : ${data.points_apres} pts`);
      }
    });
    return () => unsub();
  }, [client?.num_carte]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      responseListener.current = Notifications.addNotificationResponseReceivedListener(() => setOnglet("notifs"));
    } catch { }
    return () => {
      try {
  ...
  const handleAnniversaire = async () => {
    ...
    await supabase.from("clients").update({ points: newPoints }).eq("id", client.id);
    setClient((c: any) => ({ ...c, points: newPoints }));
    await supabase.from("notifications").insert({
      client_id: client.id, titre: "🎂 Joyeux anniversaire !",
      message: `Bon anniversaire ${client.prenom} ! +100 pts offerts aujourd'hui !`, lu: false,
    });
    Alert.alert("🎂 Joyeux anniversaire !", `+100 pts offerts ${client.prenom} !`);
  };
  ...
  const addPoints = async (pts: number, livres: any[], total: number) => {
    ...
    for (const livre of livres) {
      await supabase.from("achats").insert({
        client_id: client.id, titre: livre.titre, auteur: livre.auteur,
        prix: livre.prix, points_gagnes: Math.round(livre.prix * 10),
      });
    }
    if (client.odoo_id) {
      const avecOdooId = livres.filter(l => l.odoo_id);
      if (avecOdooId.length > 0) await odooCreateVente(client.odoo_id, avecOdooId);
    }
    await supabase.from("notifications").insert({
      client_id: client.id, titre: "🛍️ Achat confirmé !",
      message: `+${pts} points fidélité. Total : ${total.toFixed(2)} DH`, lu: false,
    });
    setClient((c: any) => ({ ...c, points: newPoints }));
    if (client?.email) await envoyerEmailAchat(client.email, client.prenom, livres, total, pts);
  };
    ...
    <View style={{ flex: 1, backgroundColor: "#F5F7FF" }}>
      {showValidation && (
        <View style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, zIndex: 999 }}>
          <EcranValidationCommande
            panier={panierValidation} client={client} fraisLivraison={15}
            onConfirmer={(cmd: any) => {
      ...
      <View style={s.header}>
        ...
        <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: wsConnecte ? "#27AE60" : "#E74C3C", marginRight: 8 }} />
      ...
      {/* Contenu */}
      {onglet === "accueil" && <OngletAccueil client={client} onAnniversaire={handleAnniversaire} wsConnecte={wsConnecte} />}
      {onglet === "boutique" && <OngletBoutique client={client} onAchat={addPoints} onValider={(panier) => { setPanierValidation(panier); setShowValidation(true); }} />}
      ...
      {onglet === "coups_coeur" && <OngletCoupsCoeur client={client} livres={LIVRES_STATIQUES} onAjouterPanier={() => {}} />}
      {onglet === "bons" && <OngletBonsReduction client={client} />}
      {onglet === "commandes" && <OngletSuiviCommandes client={client} />}
      {onglet === "scolaires" && <OngletListesScolaires onAjouterPanier={() => {}} />}
      ...
      {onglet === "plus" && (
        <ScrollView style={{ flex: 1, backgroundColor: "#F5F7FF" }}>
          ...
          {[
            ...
            <TouchableOpacity key={i} onPress={() => setOnglet(item.id)} style={{ flexDirection: "row", alignItems: "center", backgroundColor: "#fff", marginHorizontal: 16, marginBottom: 10, borderRadius: 18, padding: 18, shadowColor: "#0A2463", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2, gap: 14 }}>
...
const s = StyleSheet.create({
  ...
  rechercheBox: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", margin: 12, borderRadius: 16, paddingHorizontal: 14, paddingVertical: 10, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  rechercheInput: { flex: 1, fontSize: 14, color: "#05102A" },
  panierBox: { backgroundColor: "#fff", margin: 12, borderRadius: 20, padding: 18, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 4 },
  ...
  livreCard: { width: "47%", backgroundColor: "#fff", borderRadius: 20, padding: 16, shadowColor: "#0A2463", shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.08, shadowRadius: 10, elevation: 3 },
  ...
  detailBadge: { backgroundColor: "rgba(255,255,255,0.1)", borderRadius: 99, paddingHorizontal: 12, paddingVertical: 4 },
  ...
  detailPrixBox: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", backgroundColor: "#fff", borderRadius: 16, padding: 16, marginBottom: 14, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  anniversaireBanner: { backgroundColor: "#F5A623", margin: 16, borderRadius: 16, padding: 16, flexDirection: "row", alignItems: "center", gap: 12 },
  ...
  offreCard: { backgroundColor: "#fff", borderRadius: 18, padding: 16, marginBottom: 12, borderLeftWidth: 4, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  niveauCard: { backgroundColor: "#fff", borderRadius: 18, padding: 16, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  classementItem: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 18, padding: 16, marginBottom: 10, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2, gap: 12 },
  ...
  profilItem: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 18, padding: 16, marginBottom: 12, shadowColor: "#0A2463", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2, gap: 14 },
  ...
  notifCard: { backgroundColor: "#fff", borderRadius: 18, padding: 16, marginBottom: 10, flexDirection: "row", gap: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
  ...
  soldeBloc: { margin: 16, backgroundColor: "#05102A", borderRadius: 22, padding: 22, shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.2, shadowRadius: 16, elevation: 8 },
  ...
  menuItem: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", marginHorizontal: 16, marginBottom: 10, borderRadius: 18, padding: 18, shadowColor: "#0A2463", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2, gap: 14 },
  ...
  ticketContainer: { backgroundColor: "#fff", width: 290, borderRadius: 6, overflow: "hidden", shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 10 },
  ...
  ticketAuteur: { fontSize: 11, color: "#8AAABF", paddingHorizontal: 18, marginBottom: 4 },
  ...
  ticketPtsRow: { flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 18, paddingVertical: 4 },
